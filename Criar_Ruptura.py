...
def pasta_principal():
    main = Path.home() / "Desktop" / "Arquivos Ruptura"
    Path(main).mkdir(parents=True, exist_ok=True)
    return main
pasta_padrao = pasta_principal()


def criar_pastas_whatsapp_imagem(pasta = pasta_padrao):
    imagens_antes = pasta / 'Imagens whatsapp'
    imagens_tratadas = pasta / 'Imagens whatsapp convertidas'
    Path(imagens_antes).mkdir(parents=True, exist_ok=True)
    Path(imagens_tratadas).mkdir(parents=True, exist_ok=True)
    return imagens_antes, imagens_tratadas
criar_pastas_whatsapp_imagem()
img_antes, img_tratada = criar_pastas_whatsapp_imagem()


def criar_pasta_graficos(destino = pasta_padrao):
...
############################# Preparar imagens para o pytesseract #############################

def jpeg_para_png():
    caminho_jpg = img_antes
    pasta_destino = img_tratada

    for foto in caminho_jpg.iterdir():
        try:
            img = Image.open(foto)
            novo_caminho = pasta_destino / f"{foto.stem}.png"
            img.save(novo_caminho)   
            
        except IOError:
            print("Não foi possível abrir ou converter a imagem. Verifique o caminho do arquivo.")   
jpeg_para_png()

def imagem_para_texto(): 
    caminho = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
    pytesseract.pytesseract.tesseract_cmd = caminho
    pasta_prints = img_tratada
...
def criar_banco(df_sql, caminho = pasta_padrao):
    ...
    df_sql.to_sql(
        name='Rupturas',
        con=conexao,
        if_exists='replace', # if_exists='append' -> para juntar
        index=False
    )
